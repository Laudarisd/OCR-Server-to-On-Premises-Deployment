...
# ONNX models converted from the PaddleOCR detection and recognition models.
DET_MODEL = ONNX_DET_MODEL
REC_MODEL = ONNX_REC_MODEL
...
class OnnxModel:
    def __init__(self, model_path):
        # CPUExecutionProvider keeps inference lightweight and removes Paddle dependencies.
    ...
    def __call__(self, image):
        # ONNX Runtime needs a name-to-array feed dictionary.
...
def normalize_det(img):
    # Detection model uses ImageNet mean/std normalization from PaddleOCR.
...
def normalize_rec(img):
    # Recognition model expects pixels scaled to [-1, 1].
    img = img.astype("float32") / 255.0
    img = (img - 0.5) / 0.5
    img = img.transpose(2, 0, 1)
    return img[None].astype("float32")
...
def unclip(box, ratio=UNCLIP_RATIO):
    # DB postprocess expands text polygons before fitting the final rectangle.
...
def detect(det_model, rgb, debug_bitmap_path=None):
    ...
    # Bitmap can be saved to make ONNX/OpenVINO detector comparison easy.
    ...
    for contour in contours:
        ...
        # Map detector output coordinates back to the original image.
...
def crop_text(rgb, box):
    ...
    # Perspective crop creates a straight text line for recognition.
    crop_w = int(
        max(
    ...
    # Tall text crops are rotated just like PaddleOCR's recognizer path.
...
def get_rec_shape(rec_model):
    shape = rec_model.input_shape

    # ONNX dynamic dimensions are strings or None, so fall back to PaddleOCR defaults.
    rec_h = shape[2] if len(shape) > 2 and isinstance(shape[2], int) else DEFAULT_REC_HEIGHT
    rec_w = shape[3] if len(shape) > 3 and isinstance(shape[3], int) else None
...
def resize_rec(crop, rec_h=DEFAULT_REC_HEIGHT, rec_w=None, max_dynamic_w=MAX_DYNAMIC_REC_WIDTH):
    ...
    # White padding preserves aspect ratio while matching recognizer input width.
    padded = np.full(
        (rec_h, final_w, 3),
        255,
        dtype=np.uint8,
...
def decode_ctc(pred, chars):
    ...
    # Some exports can produce [classes, time] instead of [time, classes].
...
def draw_side_by_side_format(image_path, results, save_path):
    ...
    # Left side shows detected regions, right side shows OCR text aligned by y-position.
    ...
    canvas = Image.new("RGBA", (img_w + gap + right_w, img_h), (255, 255, 255, 255))
    ...
    for idx, result in enumerate(results, start=1):
        ...
        draw_filled_rect(draw_left, left_rect, color=color, alpha=105, outline=color, width=1)
        ...
        right_rect = (right_x1, right_y1, right_x2, right_y2)
        draw_filled_rect(draw_right, right_rect, color=color, alpha=70, outline=color, width=1)
        ...
        if use_crop_for_text:
            ...
            if crop.size > 0:
                ...
                draw_right.text((right_x1 + 4, right_y1), text, font=font, fill=(0, 0, 0, 255))
        else:
            draw_right.text((right_x1 + 4, right_y1), text, font=font, fill=(0, 0, 0, 255))
...
def process_image(image_path, det_model, rec_model, chars):
    ...
    vis_path = SAVE_DIR / f"{image_stem}_onnx.png"
    json_path = SAVE_DIR / f"{image_stem}_onnx.json"
    bitmap_path = SAVE_DIR / f"{image_stem}_onnx_debug_bitmap.png"
    crop_dir = SAVE_DIR / f"{image_stem}_onnx_debug_crops"
    ...
    if SAVE_OUTPUTS:
        ...
        for result in results:
            print(
                f'{result["text"]} | '
                f'rec={result["rec_score"]:.4f} | '
                f'det={result["det_score"]:.4f}'
            )
...
def main():
    if SAVE_OUTPUTS:
        print("Loading ONNX models on CPU...")

    det_model = OnnxModel(DET_MODEL)
    rec_model = OnnxModel(REC_MODEL)
    chars = load_dict(REC_DICT)

    if SAVE_OUTPUTS:
        print("Detection input:", det_model.input_name, det_model.input_shape)
        print("Detection output:", det_model.output_name, det_model.output_shape)
        print("Recognition input:", rec_model.input_name, rec_model.input_shape)
        print("Recognition output:", rec_model.output_name, rec_model.output_shape)
