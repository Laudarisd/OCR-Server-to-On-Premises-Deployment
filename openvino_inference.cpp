...
// Detection model path. This model finds text boxes from the full image.
static const std::string DET_MODEL = "models_openvino/PP-OCRv5_server_det_infer.xml";

// Recognition model path. This model reads text from each cropped text box.
static const std::string REC_MODEL = "models_openvino/korean_PP-OCRv5_mobile_rec.xml";
...
ov::Tensor matToDetTensor(const cv::Mat& rgb) {
    int h = rgb.rows;
    int w = rgb.cols;

    ov::Tensor tensor(ov::element::f32, ov::Shape{1, 3, static_cast<size_t>(h), static_cast<size_t>(w)});
...
ov::Tensor matToRecTensor(const cv::Mat& rgb) {
    int h = rgb.rows;
    int w = rgb.cols;
...
std::vector<DetItem> detect(ov::CompiledModel& det_model,
                            const cv::Mat& rgb,
                            const std::string& debug_bitmap_path) {
    cv::Mat resized = resizeDet(rgb);

    ov::InferRequest req = det_model.create_infer_request();
    ov::Tensor input_tensor = matToDetTensor(resized);
...
int getRecHeight(const ov::CompiledModel& rec_model) {
    ov::PartialShape pshape = rec_model.input().get_partial_shape();

    if (pshape.rank().is_static() && pshape.size() >= 4 && pshape[2].is_static()) {
        return static_cast<int>(pshape[2].get_length());
    }
...
std::pair<std::string, float> decodeCTC(const ov::Tensor& output,
...
std::pair<std::string, float> recognize(ov::CompiledModel& rec_model,
    ...
    ov::InferRequest req = rec_model.create_infer_request();
    ov::Tensor input_tensor = matToRecTensor(rec_img);
...
std::vector<OCRResult> processImage(const std::string& image_path,
    ...
    std::string crop_dir = OUTPUT_DIR + "/" + image_stem + "_openvino_debug_crops";
    std::string debug_bitmap_path = SAVE_OUTPUTS ? OUTPUT_DIR + "/" + image_stem + "_openvino_debug_bitmap.png" : "";
...
int main() {
    try {
        if (SAVE_OUTPUTS) {
            ...
            std::cout << "Loading OpenVINO models..." << std::endl;
        }

        ov::Core core;
        ov::CompiledModel det_model = core.compile_model(DET_MODEL, "CPU");
        ov::CompiledModel rec_model = core.compile_model(REC_MODEL, "CPU");
        std::vector<std::string> chars = loadDict(REC_DICT);

        if (SAVE_OUTPUTS) {
            std::cout << "Detection input: " << det_model.input().get_partial_shape() << std::endl;
            std::cout << "Recognition input: " << rec_model.input().get_partial_shape() << std::endl;
            std::cout << "Recognition output: " << rec_model.output().get_partial_shape() << std::endl;
        }
        ...
        for (const auto& image_path : image_paths) {
            ...
            if (SAVE_OUTPUTS) {
                std::string vis_path = OUTPUT_DIR + "/" + image_stem + "_openvino.png";
                std::string json_path = OUTPUT_DIR + "/" + image_stem + "_openvino.json";
                std::string crop_dir = OUTPUT_DIR + "/" + image_stem + "_openvino_debug_crops";
